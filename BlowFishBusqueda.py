...
def encriptarfichero_BlowFish():
    ...
    for nombre_fichero in os.listdir("./datos/"):
        ...
        with open(os.path.join('./datos/', nombre_fichero), 'rb') as fichero_entrada:
            with open(os.path.join('./salida/BlowFish', nombre_fichero + '.enc'), 'wb') as salida:
...
def buscar_BlowFish(palabra):
        try:
            ...
            for fichero in os.listdir('./salida/BlowFish'):
                encontrada = 0
                with open(os.path.join('./salida/BlowFish', fichero), 'rb') as entrada:
...
inicio=time.time()            
encriptarfichero_BlowFish()
fin=time.time()
print('Tiempo ivertido en encriptar',fin-inicio)
buscar_BlowFish("nombre1")
buscar_BlowFish("nombre40")